...
def login():
    ...
    if request.method == "POST":
        ...
        if not operador or not operador.verificar_senha(senha):
            ...
            return render_template("login.html", login_str=login_str)
        ...
        if operador.senha_temporaria:
            flash("Sua senha é temporária — recomendamos trocar em 'Trocar Senha'.", "warning")
...
@login_required
def trocar_senha():
    """Permite ao operador trocar sua própria senha.

    Obrigatório no 1º login (quando senha_temporaria=True).
    """
    if request.method == "POST":
...
def recuperar_senha():
    """O usuário registra um pedido; o admin redefine presencialmente."""
    if request.method == "POST":
        ...
        flash(
            "Solicitação registrada. Procure o administrador presencialmente "
